function image_file = generate_image_file_name(p)
	...
	file_dir = [p.base_path, 'Research/Crypt/Images/ParameterOptimisation/', p.chaste_test, '/', func2str(p.obj), '/'];
	if exist(file_dir,'dir')~=7
		% Make the full path
		if exist([p.base_path, 'Research/Crypt/Images/ParameterOptimisation/', p.chaste_test, '/'],'dir')~=7
			mkdir([p.base_path, 'Research/Crypt/Images/ParameterOptimisation/', p.chaste_test, '/']);
		end
