...
function plot_pspace_test(vf,cct)
...
%                 pspace(i,j) = difference;
                pspace(i,j) = ((difference - expected_difference)/expected_difference)^2;
%                 pspace(i,j) = 0.5 * ((difference - expected_difference)/expected_difference)^2 + 0.5 * ((total - expected_cell_count)/expected_cell_count)^2;
    ...
    h = figure();
    imagesc(flipud(pspace),'AlphaData',~isnan(flipud(pspace)), [0 2.5]);
    ...
    print(['/Users/phillipbrown/Research/Crypt/Images/Chaste/CellKillCount/CellKillCountVF' num2str(vf), '_CCT_' num2str(cct) ''],'-dpdf');
