...
public:
	...
	void TestDampingMutation() throw(Exception)
	{
        ...
        // ******************************************************************************************** 
        // Crypt height 
        unsigned mutantCryptHeight = n; // Number of proliferative cells, counting up from the bottom
        if(CommandLineArguments::Instance()->OptionExists("-Mn"))
        {   
            mutantProliferativeCompartment = CommandLineArguments::Instance()->GetUnsignedCorrespondingToOption("-Mn");
            PRINT_VARIABLE(mutantCryptHeight)
