classdef RigidSupport < LineSimulation
	...
	methods
		...
		function obj = RigidSupport(nCells, p, g, w, b, seed, varargin)
			% All the initilising
			obj.SetRNGSeed(seed);
			...
			%---------------------------------------------------
			% Make a layer of elements that are pinned in place
			%---------------------------------------------------
			stromaTop = -0.1;
			stromaBottom = -4;
			nodeList = Node.empty();
			...
			obj.AddSimulationModifier(   PinNodes(  nodeList  )   );
