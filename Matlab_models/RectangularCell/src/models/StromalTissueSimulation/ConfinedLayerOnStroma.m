classdef ConfinedLayerOnStroma < LineSimulation
	...
	methods
		...
		function obj = ConfinedLayerOnStroma(nCells, p, g, w, b, sae, spe, seed, varargin)
			% All the initilising
			obj.SetRNGSeed(seed);
			...
			confinedNodes = [nodeBottomLeft, nodeTopLeft];
			...
			obj.cellList.grownCellTargetArea = 0.5;
			...
			nodeBottomLeft 	= nodeBottomRight;
			nodeTopLeft 	= nodeTopRight;
			nodeTopRight 	= Node(nCells*0.5,1,obj.GetNextNodeId());
			nodeBottomRight	= Node(nCells*0.5,0,obj.GetNextNodeId());
			...
			obj.cellList(end).grownCellTargetArea = 0.5;
			confinedNodes = [confinedNodes, nodeBottomRight, nodeTopRight];
			...
			obj.AddSimulationModifier(   PinNodes(  [nodeList(1), nodeList(end-2:end), confinedNodes]  )   );
			...
			pathName = sprintf('ConfinedLayerOnStroma/n%gp%gg%gw%gb%gsae%gspe%g_seed%g/',nCells,p,g,w,b,sae,spe,seed);
